...
const Navbar = () => {
    const closeHambRef = React.useRef<HTMLDivElement>(null);
    const closeCaretRef = React.useRef<HTMLDivElement>(null);
    ...
    useEffect(() => {
        let handler = (event: MouseEvent) => {
            if(!hambRef.current?.contains(event.target as Node) || !hambRef.current?.contains(closeHambRef.current as Node) || !hambRef.current?.contains(closeCaretRef.current as Node)){
                setHamb(false);
                setShow(false);
            }
        }
    ...
    return <div className="">
        <div className="md:w-[100vw] w-full bg-slate-100">
            <nav className="flex justify-between items-center w-[100%] mx-auto pt-5 pb-5 sm:p-10 relative" ref={hambRef}>
                <div className="mr-0 sm:mr-auto flex justify-around items-center">
                    <h1 className="md:text-2xl cursor-pointer  text-lg"><Link href='/'>BlogPedia</Link></h1>
                    <div className="flex justify-between items-center relative w-[230px] sm:w-[420px] md:w-[500px] sm:ml-4 ml-1">
                ...
                <div className="flex justify-around items-center ml-[-19px] sm:ml-0 ">
                    <div ref={closeHambRef}><GiHamburgerMenu className="text-xl mr-[10px] cursor-pointer md:hidden" onClick={handleHamb}/></div>
                    <RxAvatar className="text-4xl cursor-pointer" onClick={handleClick} />
                    <div ref={closeCaretRef}><AiFillCaretDown className="cursor-pointer text-[13px]" onClick={handleClick} /></div>
                </div>
                { show &&
                    (
